...
function fileExtension(fileName: string, mimeType: string) {
  ...
  const fromMime: Record<string, string> = { "image/jpeg": ".jpg", "image/png": ".png", "image/webp": ".webp", "image/gif": ".gif", "image/avif": ".avif", "image/heic": ".heic", "image/heif": ".heif" };
  return fromMime[mimeType] || ".img";
}

export async function uploadToImageKit(input: { contentBase64: string; fileName: string; mimeType: string }) {
  ...
  uploadData.append("folder", "/yazin-link/uploads");
  ...
  if (!uploadResponse.ok) {
    ...
    throw new Error(failure?.message || "تعذر رفع الصورة. حاول مرة أخرى.");
  }

  const result = (await uploadResponse.json()) as ImageKitUploadResult;
  if (!result.url || !result.fileId) throw new Error("لم نحصل على رابط الصورة بعد الرفع.");
  return result;
}
